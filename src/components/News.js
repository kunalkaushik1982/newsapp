...
const News = (props) => {
  ...
  useEffect(() => {
    document.title=`${capatlizeFirstletter(props.category)} - NewsApp`
    updateNews(); 
  ...
  const fetchMoreData = async () => {
   ...
   setTotalResults(parsedData.totalResults)
    ...
    return (
        <>
          <h1 className="text-center" style={{margin: '35px 0px', marginTop:'90px'}}>NewsApp - Top {capatlizeFirstletter(props.category)} Headlines</h1>
          {loading && <Spinner/>}       
